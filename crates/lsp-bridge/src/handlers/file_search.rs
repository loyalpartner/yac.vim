...
impl FileSearchHandler {
    ...
    fn calculate_score(&self, file_path: &str, query: &str) -> f64 {
        if query.is_empty() {
            return 1.0;
        }
...
impl Handler for FileSearchHandler {
    ...
    async fn handle(
        ...
        debug!(
            "File search request: query='{}', page={}, page_size={}",
            input.query, input.page, input.page_size
        );
        ...
        let mut scored_files: Vec<_> = all_files
            ...
            .filter_map(|path| {
                ...
                let score = self.calculate_score(&relative_path, &input.query);
