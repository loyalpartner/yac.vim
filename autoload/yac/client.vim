...
if get(g:, 'lsp_bridge_debug', 0)
  echohl WarningMsg | echo '[yac.nvim] Enable g:lsp_bridge_debug could impact your vim experience' | echohl None
  let $LSP_BRIDGE_LOG_LEVEL = 'debug'
  if exists('$LSP_BRIDGE_LOG_FILE')
    let s:logfile = resolve($LSP_BRIDGE_LOG_FILE)
  else
    let s:logfile = tempname()
    let $LSP_BRIDGE_LOG_FILE = s:logfile
  endif
endif

" create a client
function! yac#client#create(name, command)
  let client = {}
...
function! s:start() dict
  ...
  if !isdirectory(getcwd())
    echoerr '[yac.nvim] Current cwd is not a valid directory.'
    return
  endif
  let tmpdir = fnamemodify(tempname(), ':p:h')
  if s:is_vim
    if get(g:, 'lsp_bridge_debug', 0)
      let file = tmpdir . '/yac.log'
      call ch_logfile(file, 'w')
      echohl MoreMsg | echo '[yac.nvim] channel log to '.file | echohl None
    endif
    let options = {
          \ 'noblock': 1,
          \ 'in_mode': 'json',
...
function! s:on_stderr(name, msgs)
  if get(g:, 'yac_vim_leaving', 0) | return | endif
  let data = filter(copy(a:msgs), '!empty(v:val)')
  if empty(data) | return | endif
  let client = a:name ==# 'yac' ? '[yac.nvim]' : '['.a:name.']'
  let data[0] = client.': '.data[0]
  if a:name ==# 'yac' && len(filter(copy(data), 'v:val =~# "SyntaxError: "'))
    call yac#client#check_version()
    return
  endif
  if get(g:, 'yac_disable_uncaught_error', 0) | return | endif
  call s:on_error(a:name, data)
endfunction

function! yac#client#check_version() abort
...
function! yac#client#get_client(name) abort
  return get(s:clients, a:name, v:null)
endfunction

function! yac#client#get_channel(client)
  if s:is_vim
...
function! s:request(method, args) dict
  let channel = yac#client#get_channel(self)
...
function! yac#client#is_running(name) abort
  let client = get(s:clients, a:name, v:null)
...
function! yac#client#stop(name) abort
  let client = get(s:clients, a:name, v:null)
  if empty(client) | return 1 | endif
  let running = yac#client#is_running(a:name)
  if !running
  ...
  sleep 200m
  if yac#client#is_running(a:name)
...
function! yac#client#kill(name) abort
  let client = get(s:clients, a:name, v:null)
  if empty(client) | return 1 | endif
  let running = yac#client#is_running(a:name)
...
function! yac#client#request(name, method, args)
  let client = get(s:clients, a:name, v:null)
  if !empty(client)
...
function! yac#client#notify(name, method, args)
  let client = get(s:clients, a:name, v:null)
  if !empty(client)
...
function! yac#client#request_async(name, method, args, cb)
  let client = get(s:clients, a:name, v:null)
  if !empty(client)
...
function! yac#client#on_response(name, id, resp, isErr)
  let client = get(s:clients, a:name, v:null)
  if !empty(client)
...
function! yac#client#restart(name) abort
  let stopped = yac#client#stop(a:name)
...
function! yac#client#restart_all()
  for key in keys(s:clients)
    call yac#client#restart(key)
  endfor
endfunction

function! yac#client#open_log()
  if !get(g:, 'lsp_bridge_debug', 0)
    echohl Error | echon '[yac.nvim] use let g:lsp_bridge_debug = 1 in your vimrc to enable debug mode.' | echohl None
    return
  endif
