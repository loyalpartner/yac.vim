...
pub const DapClient = struct {
    ...
    /// Send a pre-serialized DAP request and return the sequence number.
    pub fn request(self: *DapClient, req: protocol.Wire) !u32 {
        ...
        log.debug("DAP request [{d}]: {s}", .{ seq, req.command });
        return seq;
    }
    ...
    pub fn initialize(self: *DapClient) !u32 {
        ...
        defer arena.deinit();
        return self.request(try (types.Initialize{ .arguments = .{} }).wire(arena.allocator()));
    }

    /// Send configurationDone after setting breakpoints.
    pub fn sendConfigurationDone(self: *DapClient) !u32 {
        return self.request(.{ .command = "configurationDone", .arguments = .null });
    }
    ...
    pub fn sendLaunch(self: *DapClient, allocator: Allocator, program: []const u8, module: ?[]const u8, args_list: ?Value, stop_on_entry: bool, cwd: ?[]const u8, env_json: ?[]const u8, extra_json: ?[]const u8) !u32 {
        ...
        return self.request(.{ .command = "launch", .arguments = .{ .object = map } });
    }
    ...
    pub fn sendAttach(self: *DapClient, allocator: Allocator, pid: ?u32, program: ?[]const u8, extra_json: ?[]const u8) !u32 {
        ...
        return self.request(.{ .command = "attach", .arguments = .{ .object = map } });
    }
    ...
    pub fn sendSetBreakpoints(self: *DapClient, allocator: Allocator, file_path: []const u8, breakpoints: []const BreakpointInfo) !u32 {
        ...
        return self.request(.{ .command = "setBreakpoints", .arguments = arguments });
    }
    ...
    pub fn sendSetExceptionBreakpoints(self: *DapClient, allocator: Allocator, filters: []const []const u8) !u32 {
        ...
        return self.request(.{ .command = "setExceptionBreakpoints", .arguments = arguments });
    }

    /// Send threads request to get all threads.
    pub fn sendThreads(self: *DapClient) !u32 {
        return self.request(.{ .command = "threads", .arguments = .null });
    }
    ...
    pub fn sendContinue(self: *DapClient, thread_id: u32) !u32 {
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        return self.request(try (types.Continue{ .arguments = .{ .threadId = thread_id } }).wire(arena.allocator()));
    }
    ...
    pub fn sendNext(self: *DapClient, thread_id: u32) !u32 {
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        return self.request(try (types.Next{ .arguments = .{ .threadId = thread_id } }).wire(arena.allocator()));
    }
    ...
    pub fn sendStepIn(self: *DapClient, thread_id: u32) !u32 {
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        return self.request(try (types.StepIn{ .arguments = .{ .threadId = thread_id } }).wire(arena.allocator()));
    }
    ...
    pub fn sendStepOut(self: *DapClient, thread_id: u32) !u32 {
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        return self.request(try (types.StepOut{ .arguments = .{ .threadId = thread_id } }).wire(arena.allocator()));
    }
    ...
    pub fn sendStackTrace(self: *DapClient, thread_id: u32) !u32 {
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        return self.request(try (types.StackTrace{ .arguments = .{ .threadId = thread_id } }).wire(arena.allocator()));
    }
    ...
    pub fn sendScopes(self: *DapClient, frame_id: u32) !u32 {
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        return self.request(try (types.Scopes{ .arguments = .{ .frameId = frame_id } }).wire(arena.allocator()));
    }
    ...
    pub fn sendVariables(self: *DapClient, variables_ref: u32) !u32 {
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        return self.request(try (types.Variables{ .arguments = .{ .variablesReference = variables_ref } }).wire(arena.allocator()));
    }
    ...
    pub fn sendEvaluate(self: *DapClient, expression: []const u8, frame_id: ?u32, context: []const u8) !u32 {
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        return self.request(try (types.Evaluate{ .arguments = .{
    ...
    pub fn sendDisconnect(self: *DapClient, terminate_debuggee: bool) !u32 {
        var arena = std.heap.ArenaAllocator.init(self.allocator);
        defer arena.deinit();
        return self.request(try (types.Disconnect{ .arguments = .{ .terminateDebuggee = terminate_debuggee } }).wire(arena.allocator()));
    }

    /// Send terminate request (graceful).
    pub fn sendTerminate(self: *DapClient) !u32 {
        return self.request(.{ .command = "terminate", .arguments = .null });
    }
