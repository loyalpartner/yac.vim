...
/// Set the minimum log level at runtime (for dynamic level switching via handler).
...
pub fn initWithArgs(cli_level: ?Level, cli_log_file: ?[]const u8) void {
    if (cli_level) |level| {
        min_level = level;
    } else if (compat.getenv("YAC_LOG_LEVEL")) |level_str| {
...
/// Initialize with defaults (env var only). Convenience for tests.
pub fn init() void {
